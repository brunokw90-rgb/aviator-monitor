...
def build_table_html(df: pd.DataFrame, limit: int = 50) -> str:
    """CORRIGIDO: Ordenação consistente por round + datetime"""
    if df is None or df.empty:
        return "<em>Sem dados</em>"

    # Encontra coluna round
    round_col = None
    for c in df.columns:
        if c.lower() in ("round", "rodada", "id"):
            round_col = c
    ...
    if round_col and round_col in df.columns:
        # SOLUÇÃO: Ordena por round DESC, depois por datetime DESC para desempate
        if 'datetime' in df.columns:
            show = df.sort_values([round_col, 'datetime'], ascending=[False, False]).head(limit).copy()
            print(f"[TABLE] Ordenado por {round_col} DESC + datetime DESC")
        else:
            show = df.sort_values(round_col, ascending=False).head(limit).copy()
            print(f"[TABLE] Ordenado por {round_col} DESC apenas")
        
        if not show.empty:
            print(f"[TABLE] Primeiro round: {show.iloc[0][round_col]}")
    else:
        # Fallback: usar index invertido
        show = df.iloc[::-1].head(limit).copy()
        print("[TABLE] Usando ordenação por index invertido")
    
    # Formatação de datetime melhorada
    if 'datetime' in show.columns:
        def format_datetime(dt):
            if pd.isna(dt):
                return "N/A"
            if hasattr(dt, 'strftime'):
                # Mostra microssegundos para debug de ordenação
                if dt.date() == datetime.now().date():
                    return dt.strftime("%H:%M:%S.%f")[:-3]  # Remove 3 últimos dígitos dos microssegundos
                else:
...
@login_required
def db_last():
    """CORRIGIDO: Ordenação tripla para máxima consistência"""
    try:
        ...
        with eng.connect() as conn:
            # CRÍTICO: Ordenação tripla: round DESC, datetime DESC, id DESC
            result = conn.execute(text("""
                SELECT id, round, multiplier, datetime, source 
                FROM multipliers 
                ORDER BY round DESC NULLS LAST, datetime DESC NULLS LAST, id DESC 
                LIMIT :limit_val
            """), {"limit_val": limit})
...
def debug_timestamps():
    """Debug para verificar distribuição de timestamps"""
    try:
        eng = db_engine()
        if not eng:
            return jsonify({"ok": False, "error": "Banco não disponível"}), 500
        ...
        with eng.connect() as conn:
            result = conn.execute(text("""
                SELECT 
                    round, 
                    datetime, 
                    EXTRACT(EPOCH FROM datetime) as timestamp_epoch,
                    LAG(datetime) OVER (ORDER BY round DESC) as prev_datetime,
                    EXTRACT(EPOCH FROM datetime) - EXTRACT(EPOCH FROM LAG(datetime) OVER (ORDER BY round DESC)) as time_diff
                FROM multipliers 
            ...
            for row in result:
                records.append({
                    ...
                    "timestamp_epoch": float(row[2]) if row[2] else None,
                    "prev_datetime": row[3].isoformat() if row[3] else None,
                    "time_diff_seconds": float(row[4]) if row[4] else None
                })
            
        return jsonify({
            "ok": True,
            "records": records,
            "note": "Verifica se timestamps estão únicos e bem distribuídos"
        })
